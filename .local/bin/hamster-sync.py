...
cur.execute('''
select facts.id as id, activities.name as act, activity_id, start_time as st, end_time as et, description, tags.name as tag, tag_id from facts 
	...
	where 
	not exists (select 1 from dst.facts where strftime('%Y-%m-%d %H:%M', dst.facts.start_time) = strftime('%Y-%m-%d %H:%M', st)) 
	and end_time is not null order by st desc
''')
